...
describe("Place Order for Shopify (shop system) Stripe (Payment method)- CompanyID (5330-696b3762cp)", function () {
  const checkoutPageElement = new CheckoutPageObject();
  const molliePayPageObject = new MolliePayPageObject();
  const apiKey = apiKeyCartId.shopifyStripe_api_key;
  const cartId = apiKeyCartId.shopifyStripe_cart_id;

  setupFixtureAndVisitUrl(apiKey, cartId);

  it("Complete an Order for Shopify Stripe and Validate in CMS Database", function () {
    cy.url()
      .should("include", apiKeyCartId.shopifyStripe_api_key)
      .and("include", apiKeyCartId.shopifyStripe_cart_id);
  ...
  cy.get('iframe[title*="Secure payment input frame"]')
    ...
    .then(($iframe) => {
      ...
      cy.wrap($body).find('input[name="iban"]').click().type("DE89370400440532013000");
    });
