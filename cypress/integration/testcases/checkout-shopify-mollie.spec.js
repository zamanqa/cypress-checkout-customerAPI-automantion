...
describe("Place Order for Shopify (shop system) Mollie (Payment method)- CompanyID (4a56-51693630fa)", function () {
  const checkoutPageElement = new CheckoutPageObject();

  const apiKey = apiKeyCartId.shopifyMollie_api_key;
  const cartId = apiKeyCartId.shopifyMollie_cart_id;

  setupFixtureAndVisitUrl(apiKey, cartId);

  it("Complete an Order for Shopify Mollie and Validate in CMS Database", function () {
    cy.url()
